...
export async function POST(request: NextRequest) {
  ...
  try {
    chunks = await searchChunks(lastUser.content, 8);
  } catch {
    // DB may not have chunks yet — still respond but with no context
  }
  ...
  const contextBlock =
    chunks.length > 0
      ? chunks
          .map((c, i) => `[${i + 1}] File: ${c.file_path}\n${c.chunk_text}`)
          .join("\n\n---\n\n")
      : "No relevant document excerpts found.";

  const systemPrompt = `You are a helpful assistant for a confidential investor data room.
Answer the user's question using only the document excerpts provided below.
Always mention which document(s) your answer comes from (use the file path as the reference).
